...
(function() {
    ...
    function updateProgressStages(progressText) {
        ...
        // Ищем префикс источника в разных вариантах
        const yandexPattern = /^(Yandex|Яндекс):\s*/i;
        const gisPattern = /^(2GIS|2ГИС):\s*/i;
        
        if (progressText.match(yandexPattern)) {
            source = 'yandex';
            cleanProgressText = progressText.replace(yandexPattern, '');
            console.log('Detected source: yandex, clean text:', cleanProgressText);
        } else if (progressText.match(gisPattern)) {
            source = '2gis';
            cleanProgressText = progressText.replace(gisPattern, '');
